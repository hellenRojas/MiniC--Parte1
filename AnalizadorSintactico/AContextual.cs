...
{
    TablaSimbolos table;
    public int tipoActual = 0;

    public AContextual()
    {
        table = new TablaSimbolos();
    }
    private string tipos(int tipo)
    {
        string t = "";
        switch (tipo)
        {
            case 0: { t = "UNDEFINED"; break; }
            case 1: { t = "INT"; break; }
    ...
    public override Object VisitExprAST([NotNull] Parser1.ExprASTContext context)
    {
        ...
        tipo1 = (int)Visit(context.term(0));

        for (int i = 1; i <= context.term().Length - 1; i++)
        {

            tipo2 = (int)Visit(context.term(i));

            if ((tipo1 != 1) || (tipo2 != 1))
            {
                Console.WriteLine("Error:El tipo no es INT");
                break;
            }
            tipo1 = tipo2;
    ...
    public override Object VisitAssignstatAST([NotNull] Parser1.AssignstatASTContext context)
    {
        int tipoExpr = (int)Visit(context.expression());
        IToken aux = table.buscar(context.ID().GetText());
        int tipoID;
        ...
        {
            Console.WriteLine("Variable " + context.ID().GetText() + " no definida");
            tipoID = 0;
        }

        if (tipoExpr != tipoID)
        {
            Console.WriteLine("Tipos Incompatibles (" + tipos(tipoExpr) + "," + tipos(tipoID) + ")");
        }
        return null;
    }


    public override Object VisitIdlistAST([NotNull] Parser1.IdlistASTContext context)
    {
        for (int i = 0; i <= context.ID().Length - 1; i++)
        {
            table.insertar(context.ID(i).GetText(), tipoActual);
        }
    ...
    public override Object VisitStatsAST([NotNull] Parser1.StatsASTContext context)
    {
    ...
    public override Object VisitTermAST([NotNull] Parser1.TermASTContext context)
    {
        ...
        tipo1 = (int)Visit(context.factor(0));

        for (int i = 1; i <= context.factor().Length - 1; i++)
        {

            tipo2 = (int)Visit(context.factor(i));
    ...
    public override Object VisitCondeclAST([NotNull] Parser1.CondeclASTContext context)
    {
    ...
    public override Object VisitSindeclAST([NotNull] Parser1.SindeclASTContext context)
    {
    ...
    public override Object VisitStrfactorAST([NotNull] Parser1.StrfactorASTContext context)
    {
        return 2;
    }

    public override Object VisitIdfactorAST([NotNull] Parser1.IdfactorASTContext context)
    {
        IToken aux = table.buscar(context.ID().GetText());
        if (aux != null)
            return aux.Type;
    ...
    public override Object VisitIntdeclAST([NotNull] Parser1.IntdeclASTContext context)
    {
        tipoActual = 1;
        context.INT();
    ...
    public override Object VisitStrdeclAST([NotNull] Parser1.StrdeclASTContext context)
    {
        tipoActual = 2;
        context.STRING();
        Visit(context.idlist());
    ...
    public override Object VisitDelcsAST([NotNull] Parser1.DelcsASTContext context)
    {

        Visit(context.declaration(0));
        for (int i = 1; i <= context.declaration().Length - 1; i++)
        {
    ...
    public override Object VisitProgram([NotNull] Parser1.ProgramContext context)
    {
        return VisitChildren(context);
    }
...
class PrettyPrint : Parser1BaseVisitor<Object>
{
    ...
    public override object VisitStrFactorAST([NotNull] Parser1.StrFactorASTContext context)
    {
        TreeNode str = new TreeNode(context.STRI().ToString());
        TreeNode[] arreglo = new TreeNode[] { str };
        TreeNode final = new TreeNode("Factor-String", arreglo);
        return final;
    }
